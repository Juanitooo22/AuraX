...
CHATTERBOX_TTS_LOCK = threading.Lock()
...
def tts_fast():
    """
    TTS principal de AuraX usando Chatterbox.
    Devuelve WAV directamente al navegador.
    """
    if request.method == 'OPTIONS':
        r = jsonify({})
        r.headers['Access-Control-Allow-Origin'] = '*'
    ...
    try:
        data = request.get_json(silent=True) or {}
        texto = limpiar_texto_tts(data.get('texto') or '').strip()

        if not texto:
            return jsonify({'error': 'no texto'}), 400

        print(f"🎙️ CHATTERBOX: {texto}")

        with torch.inference_mode():
            # Chatterbox estándar usa max_new_tokens=1000.
            # Para voz conversacional de AuraX usamos una versión limitada
            # para reducir muchísimo la latencia.
            with CHATTERBOX_TTS_LOCK:
        ...
        # Guardamos temporalmente el WAV en memoria.
        buffer = io.BytesIO()
        torchaudio.save(
            buffer,
            wav.detach().cpu(),
        ...
        print(
            f"✅ CHATTERBOX generado | "
            f"{len(audio)/1024:.1f} KB | "
            f"{CHATTERBOX_MODEL.sr} Hz | "
        ...
        return Response(
            ...
            headers={
                'Access-Control-Allow-Origin': '*',
                'Cache-Control': 'no-store',
                'Content-Disposition': 'inline; filename=aurax.wav'
            }
        )
